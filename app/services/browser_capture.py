...
async def capture_chapter_images(
    ...
    command = build_capture_worker_command(
        ...
        right_arrow_stable_rounds=settings.browser_right_arrow_stable_rounds,
        right_arrow_presses_per_round=settings.browser_right_arrow_presses_per_round,
        right_arrow_stop_on_url_change=settings.browser_right_arrow_stop_on_url_change,
    )
    ...
    return ChapterCaptureResponse(
        ...
        diagnostics=ChapterCaptureDiagnostics(
            ...
            rightArrowStepsExecuted=int(payload.get("rightArrowStepsExecuted", 0)),
            rightArrowPressesSent=int(payload.get("rightArrowPressesSent", 0)),
            rightArrowSequenceBefore=int(payload.get("rightArrowSequenceBefore", 0)),
            rightArrowSequenceAfter=int(payload.get("rightArrowSequenceAfter", 0)),
            rightArrowGrowthEvents=int(payload.get("rightArrowGrowthEvents", 0)),
            rightArrowStableRounds=int(payload.get("rightArrowStableRounds", 0)),
            rightArrowStopReason=str(payload.get("rightArrowStopReason", "none")),
            rightArrowProductive=bool(payload.get("rightArrowProductive", False)),
            rightArrowUrlChanged=bool(payload.get("rightArrowUrlChanged", False)),
            rightArrowInitialUrl=str(payload.get("rightArrowInitialUrl", "")),
            rightArrowFinalUrl=str(payload.get("rightArrowFinalUrl", "")),
...
def build_capture_worker_command(
    ...
    right_arrow_stable_rounds: int,
    right_arrow_presses_per_round: int,
    right_arrow_stop_on_url_change: bool,
) -> list[str]:
    ...
    command = [
        ...
        str(right_arrow_max_steps),
        "--right-arrow-wait-ms",
        str(right_arrow_wait_ms),
        "--right-arrow-stable-rounds",
        str(right_arrow_stable_rounds),
        "--right-arrow-presses-per-round",
        str(right_arrow_presses_per_round),
        "--right-arrow-stop-on-url-change",
        "true" if right_arrow_stop_on_url_change else "false",
    ]
    if browser_executable_path:
